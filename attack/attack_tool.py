...
def pgd_window_attack(model, x, y, window_size=(32, 32), n_iter=10, step_size=0.03):
    ...
    noise = torch.randn((b, c, window_size[0] * window_size[1]), device=x.device) * step_size
...
def pgd_pixels_attack(model, x, y, nums, n_iter=10, step_size=0.07):
    ...
    noise = torch.randn((b, c, nums), device=x.device) * step_size
...
def pgd_lines_attack(model, x, y, nums, direction='vertical', n_iter=10, step_size=0.07):
    ...
    if direction == 'vertical':
        ...
        rows = torch.arange(w)
        noise = torch.randn((b, c, nums * w), device=x.device) * step_size
    else:
        cols = torch.arange(h)
        rows = torch.randperm(w)[:nums]
        noise = torch.randn((b, c, nums * h), device=x.device) * step_size
...
def fgsm_lines_attack(model, x, y, nums, epsilon=0.5, direction='vertical'):
    ...
    if direction == 'vertical':
        cols = torch.randperm(h)[:nums]
        rows = torch.arange(w)
        noise = torch.randn((b, c, nums * w), device=x.device) * epsilon
    else:
        cols = torch.arange(h)
        rows = torch.randperm(w)[:nums]
        noise = torch.randn((b, c, nums * h), device=x.device) * epsilon
...
def fgsm_pixels_attack(model, x, y, nums=20, epsilon=0.5):
    ...
    noise = torch.randn((b, c, nums), device=x.device) * epsilon
...
def fgsm_window_attack(model, x, y, window_size=(32, 32), epsilon=0.3):
    ...
    noise = torch.randn((b, c, window_size[0] * window_size[1]), device=x.device) * epsilon
